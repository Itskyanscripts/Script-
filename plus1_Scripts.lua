...
local function savePendingKey(key, expireAt)
	...
	getgenv().plus1_pending_key = key
	getgenv().plus1_pending_expire = expireAt
end

local function loadPendingKey()
	local key, expire = getgenv().plus1_pending_key, getgenv().plus1_pending_expire
	pcall(function()
...
local function isUnlocked()
	...
	if key and key ~= MASTER_KEY and expire then
		if os.time() <= expire then
			...
			getgenv().plus1_unlocked = false
			getgenv().plus1_key = nil
			getgenv().plus1_expire = nil
			pcall(function()
...
SubmitBtn.MouseButton1Click:Connect(function()
	...
	if pending and entered == pending then
		...
		getgenv().plus1_pending_key = nil
		getgenv().plus1_pending_expire = nil
...
task.spawn(function()
	while true do
		...
		elseif getgenv().plus1_unlocked and key and expire and os.time() > expire then
			getgenv().plus1_unlocked = false
			getgenv().plus1_key = nil
			getgenv().plus1_expire = nil
