...
local function buildGame2Features()
	...
	-- World 3 (fixed - uses index 172, also handles TouchInterest child)
	...
	createToggle(FeaturesFrame, "Auto Farm", 154, function(Value)
		...
		while getgenv().g2_w3 do
			...
			pcall(function()
				...
				local children = world3:GetChildren()
				local target = children[172]
				if target then
					fireTouch(target)
				end
				-- Fallback: fire any part in World3 that has TouchInterest
				if not target then
					for _, child in ipairs(children) do
						if child:FindFirstChild("TouchInterest") or child:IsA("BasePart") then
