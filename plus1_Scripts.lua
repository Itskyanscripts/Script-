...
local function buildGame2Features()
	...
	createToggle(FeaturesFrame, "Auto Farm", 154, function(Value)
		...
		while getgenv().g2_w3 do
			task.wait(0.15)
			local ok, part = pcall(function()
				return workspace.World3:GetChildren()[172]
			end)
			if ok and part then
				fireTouch(part)
			end
	...
	-- TP World Area
	createSection(FeaturesFrame, "TP World Area", 260)
	createButton(FeaturesFrame, "World 1", 284, function()
		local char = player.Character
		if char and char:FindFirstChild("HumanoidRootPart") then
			char:MoveTo(Vector3.new(1109, 670, -401))
		end
	end)
	createButton(FeaturesFrame, "World 2", 322, function()
		local char = player.Character
		if char and char:FindFirstChild("HumanoidRootPart") then
			char:MoveTo(Vector3.new(-1755, 670, -413))
		end
	end)
	createButton(FeaturesFrame, "World 3", 360, function()
		local char = player.Character
		if char and char:FindFirstChild("HumanoidRootPart") then
	...
	FeaturesFrame.CanvasSize = UDim2.new(0, 0, 0, 410)
end
