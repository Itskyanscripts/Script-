...
-- Safe ENV table (works with or without getgenv)
local ENV = _G
pcall(function()
...
-- Early sound stubs (real implementations assigned later)
local playClick, playWrong, playFind, playLevelUp, playUnlock, playExpire, playWin
playClick = function() end
playWrong = function() end
playFind = function() end
playLevelUp = function() end
playUnlock = function() end
playExpire = function() end
playWin = function() end
...
UIStroke.Color = Color3.fromRGB(0, 255, 70) -- green
UIStroke.Thickness = 2
UIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
UIStroke.Parent = Main
-- subtle black inner edge look
...
OpenStroke.Color = Color3.fromRGB(0, 255, 80)
...
-- ========== MATRIX RAIN (1 0 0 1) on Main - behind everything ==========
do
	Main.ClipsDescendants = true
	local RainLayer = Instance.new("Frame")
	RainLayer.Name = "MatrixRain"
			...
			if Main.Visible then
				...
				task.spawn(function()
					while d and d.Parent and y < 320 do
						y = y + speed * 2.2
						d.Position = UDim2.new(0, x, 0, y)
						d.TextTransparency = math.clamp(0.4 + (y / 400), 0.4, 0.9)
						task.wait(0.03)
					end
...
local function createToggle(parent, name, y, callback)
	...
	toggle.MouseButton1Click:Connect(function()
		...
		if enabled then
			TweenService:Create(toggle, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(0, 170, 70)}):Play()
			TweenService:Create(circle, TweenInfo.new(0.15, Enum.EasingStyle.Back), {
				Position = UDim2.new(1, -19, 0.5, -8),
				BackgroundColor3 = Color3.fromRGB(200, 255, 210)
			}):Play()
		else
			TweenService:Create(toggle, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(45, 45, 45)}):Play()
			TweenService:Create(circle, TweenInfo.new(0.15, Enum.EasingStyle.Back), {
...
local function createButton(parent, name, y, callback)
	...
	stroke.Color = Color3.fromRGB(0, 140, 55)
	stroke.Thickness = 0
	stroke.Parent = btn

	btn.MouseEnter:Connect(function()
		TweenService:Create(btn, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(0, 140, 55)}):Play()
	...
	btn.MouseLeave:Connect(function()
		...
		TweenService:Create(stroke, TweenInfo.new(0.12), {Thickness = 0}):Play()
	end)
	btn.MouseButton1Click:Connect(function()
		...
		task.delay(0.07, function()
			if btn and btn.Parent then
				TweenService:Create(btn, TweenInfo.new(0.12, Enum.EasingStyle.Back), {Size = orig}):Play()
			end
		end)
		callback()
	end)
...
-- Slider (1-500) connected to input
...
local sliding = false
local setWalkSpeed -- forward declare
local speedOn = false
local function updateSliderFromValue(val)
	val = math.clamp(tonumber(val) or 16, 1, 500)
	local pct = (val - 1) / 499
	SliderFill.Size = UDim2.new(pct, 0, 1, 0)
	SliderKnob.Position = UDim2.new(pct, -8, 0.5, -8)
	SpeedInput.Text = tostring(math.floor(val))
	SpeedLabel.Text = "Current: " .. tostring(math.floor(val))
	return val
end
...
SliderBG.InputBegan:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		sliding = true
		pcall(playClick)
		local val = updateSliderFromValue(valueFromMouse(input.Position.X))
		if speedOn then setWalkSpeed(val) end
	end
end)
...
-- WalkSpeed toggle (ON = custom speed, OFF = 16)
...
local function applySpeedToggle(on)
	speedOn = on
	if on then
...
SpeedToggle.MouseButton1Click:Connect(function()
	pcall(playClick)
	applySpeedToggle(not speedOn)
end)

SpeedInput.FocusLost:Connect(function(enter)
	local val = updateSliderFromValue(SpeedInput.Text)
	if speedOn then
		setWalkSpeed(val)
	end
end)
...
player.CharacterAdded:Connect(function(char)
	...
	if speedOn then
		...
		if hum then
			hum.WalkSpeed = math.clamp(tonumber(SpeedInput.Text) or 16, 1, 500)
		end
	end
end)
...
InfF.Position = UDim2.new(0,0,0,160)
...
local FeedbackPage, ViewPage, FeedbackTab, ViewTab -- filled later
local function switchTab(selected)
	...
	if FeedbackPage then FeedbackPage.Visible = (selected == "Feedback") end
	if ViewPage then ViewPage.Visible = (selected == "View") end

	local tabs = { ["Game"] = GameTab, ["Game List"] = GameListTab, ["Misc"] = MiscTab }
	if FeedbackTab then tabs["Feedback"] = FeedbackTab end
...
GameListTab.MouseButton1Click:Connect(function()
	pcall(playClick)
	switchTab("Game List")
end)
...
if SUPPORTED[game.PlaceId] then
	...
	local okBuild, errBuild = pcall(SUPPORTED[game.PlaceId])
	if not okBuild then
		warn("[+1 Scripts] feature build error:", errBuild)
	end
else
	NotAddedLabel.Visible = true
	FeaturesFrame.Visible = false
...
pcall(function() switchTab("Game") end)
...
-- Sounds (ASMR / UI clicks — reliable public assets)
local clicks = {
	"rbxassetid://6042053626",
	"rbxassetid://6895079853",
	"rbxassetid://911382214",
	"rbxassetid://421058925",
}
local wrongs = {"rbxassetid://138081500", "rbxassetid://178077629"}
local finds = {"rbxassetid://515373460", "rbxassetid://4612375201"}
local levels = {"rbxassetid://3398620867", "rbxassetid://6026984224"}
local unlocks = {"rbxassetid://4612375201", "rbxassetid://3398620867"}
local wins = {"rbxassetid://183990826", "rbxassetid://3398620867"}
local expires = {"rbxassetid://138081500", "rbxassetid://178077629"}
...
local function playSfx(ids, vol, speed)
	task.spawn(function()
		local ok, err = pcall(function()
			...
			s.Volume = vol or 0.5
			s.PlaybackSpeed = speed or (0.95 + math.random() * 0.12)
			s.PlayOnRemove = false
			-- Parent where LocalScripts can hear reliably
			s.Parent = playerGui
			s:Play()
...
playClick = function() playSfx(clicks, 0.45, 1.05 + math.random() * 0.1) end
playWrong = function() playSfx(wrongs, 0.55, 0.85) end
playFind = function() playSfx(finds, 0.5, 1.1) end
playLevelUp = function() playSfx(levels, 0.5, 1.0) end
playUnlock = function() playSfx(unlocks, 0.55, 1.05) end
playExpire = function() playSfx(expires, 0.65, 0.7) end
playWin = function() playSfx(wins, 0.7, 1.0) end

-- ========== FEEDBACK (everyone) + VIEW (Eyfanboy09 only) ==========
	...
	pcall(function()
		if isfile and isfile(REQUEST_FILE) and readfile then
			for line in readfile(REQUEST_FILE):gmatch("[^\r\n]+") do
				local user, msg, ts = line:match("([^|]+)|([^|]+)|?(.*)")
				if user and msg then
					table.insert(list, {user = user, msg = msg, ts = ts or ""})
				end
			end
		end
	end)
...
local function saveRequest(user, msg)
	pcall(function()
		...
		local line = user .. "|" .. msg:gsub("|", "/") .. "|" .. tostring(os.time())
		writefile(REQUEST_FILE, existing .. line .. "\n")
	end)
end

-- Feedback page (everyone)
FeedbackPage = Instance.new("Frame")
...
local FbHint = Instance.new("TextLabel")
FbHint.Size = UDim2.new(1, 0, 0, 16)
FbHint.Position = UDim2.new(0, 0, 0, 20)
FbHint.BackgroundTransparency = 1
FbHint.Text = "Type the game name or PlaceId"
FbHint.TextColor3 = Color3.fromRGB(140, 140, 140)
FbHint.Font = Enum.Font.Gotham
FbHint.TextSize = 11
FbHint.TextXAlignment = Enum.TextXAlignment.Left
FbHint.Parent = FeedbackPage

local RequestBox = Instance.new("TextBox")
RequestBox.Size = UDim2.new(1, 0, 0, 36)
RequestBox.Position = UDim2.new(0, 0, 0, 42)
...
SendBtn.Position = UDim2.new(0, 0, 0, 86)
...
SendStatus.Position = UDim2.new(0, 0, 0, 126)
...
SendBtn.MouseEnter:Connect(function()
	TweenService:Create(SendBtn, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(0, 180, 75)}):Play()
end)
SendBtn.MouseLeave:Connect(function()
	TweenService:Create(SendBtn, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(0, 150, 60)}):Play()
end)
SendBtn.MouseButton1Click:Connect(function()
	...
	SendStatus.TextColor3 = Color3.fromRGB(0, 220, 90)
	RequestBox.Text = ""
	local orig = SendBtn.Size
	SendBtn.Size = UDim2.new(1, -6, 0, 30)
	task.delay(0.08, function()
		if SendBtn and SendBtn.Parent then
			TweenService:Create(SendBtn, TweenInfo.new(0.12, Enum.EasingStyle.Back), {Size = orig}):Play()
		end
	end)
end)

-- Feedback tab (everyone)
FeedbackTab = Instance.new("TextButton")
FeedbackTab.Name = "Feedback"
FeedbackTab.Size = UDim2.new(0, 80, 1, 0)
FeedbackTab.Position = UDim2.new(0, 216, 0, 0)
FeedbackTab.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
FeedbackTab.Text = "Feedback"
FeedbackTab.TextColor3 = Color3.fromRGB(170, 170, 170)
FeedbackTab.Font = Enum.Font.GothamMedium
FeedbackTab.TextSize = 12
FeedbackTab.Parent = TabBar
Instance.new("UICorner", FeedbackTab).CornerRadius = UDim.new(0, 6)

-- Shrink other tabs slightly to fit
GameTab.Size = UDim2.new(0, 68, 1, 0)
GameListTab.Size = UDim2.new(0, 72, 1, 0)
GameListTab.Position = UDim2.new(0, 72, 0, 0)
MiscTab.Size = UDim2.new(0, 58, 1, 0)
MiscTab.Position = UDim2.new(0, 148, 0, 0)
FeedbackTab.Position = UDim2.new(0, 210, 0, 0)

-- View page + tab (owner only)
ViewPage, ViewTab = nil, nil
if IS_OWNER then
	ViewTab = Instance.new("TextButton")
	ViewTab.Name = "View"
	ViewTab.Size = UDim2.new(0, 52, 1, 0)
	ViewTab.Position = UDim2.new(0, 294, 0, 0)
	ViewTab.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
	ViewTab.Text = "View"
	ViewTab.TextColor3 = Color3.fromRGB(170, 170, 170)
	ViewTab.Font = Enum.Font.GothamMedium
	ViewTab.TextSize = 12
	ViewTab.Parent = TabBar
	Instance.new("UICorner", ViewTab).CornerRadius = UDim.new(0, 6)

	-- Slightly tighter tabs for owner
	...
	-- Falling "Views" rain in View page
	local ViewRain = Instance.new("Frame")
	ViewRain.Size = UDim2.new(1, 0, 1, 0)
	ViewRain.BackgroundTransparency = 1
	ViewRain.ZIndex = 0
	ViewRain.ClipsDescendants = true
	ViewRain.Parent = ViewPage
	task.spawn(function()
		local words = {"Views", "1", "0", "0", "1", "View"}
		while ViewRain and ViewRain.Parent do
			if ViewPage.Visible then
				local d = Instance.new("TextLabel")
				d.Size = UDim2.new(0, 40, 0, 14)
				d.BackgroundTransparency = 1
				d.Text = words[math.random(1, #words)]
				d.TextColor3 = Color3.fromRGB(0, math.random(80, 160), math.random(40, 80))
				d.Font = Enum.Font.Code
				d.TextSize = 10
				d.TextTransparency = 0.6
				d.ZIndex = 0
				d.Parent = ViewRain
				local x, y = math.random(0, 300), -16
				d.Position = UDim2.new(0, x, 0, y)
				local speed = math.random(20, 45) / 100
				task.spawn(function()
					while d and d.Parent and y < 280 do
						y = y + speed * 2
						d.Position = UDim2.new(0, x, 0, y)
						task.wait(0.03)
					end
	...
	ViewScroll.ScrollBarImageColor3 = Color3.fromRGB(0, 160, 65)
	ViewScroll.ZIndex = 2
	...
	local function refreshViews()
		...
		if #list == 0 then
			...
			for i, req in ipairs(list) do
				...
				btn.MouseEnter:Connect(function()
					TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(0, 100, 45)}):Play()
				end)
				btn.MouseLeave:Connect(function()
					TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(28, 28, 28)}):Play()
				end)
				btn.MouseButton1Click:Connect(function()
					pcall(playClick)
					pcall(function()
						if setclipboard then setclipboard(req.msg) end
					end)
					t2.Text = "Copied!"
					task.wait(0.6)
					t2.Text = "from " .. req.user
...
-- Patch switchTab to include Feedback + View
...
local KEY_DURATION = 10 * 60  -- 10 minutes
local KEY_FILE = "plus1_key_data.txt"
local USED_FILE = "plus1_used_keys.txt"
...
local function saveKeyData(key, expireAt)
	...
	ENV.plus1_key, ENV.plus1_expire = key, expireAt
end
...
local function isKeyUsed(key)
	local used = false
	pcall(function()
		if isfile and isfile(USED_FILE) and readfile then
			for line in readfile(USED_FILE):gmatch("[^
]+") do
				if line == key then used = true break end
			end
		end
	end)
	return used
...
local function markKeyUsed(key)
	pcall(function()
		...
		local prev = ""
		if isfile and isfile(USED_FILE) and readfile then
			prev = readfile(USED_FILE)
			if prev ~= "" and not prev:match("
$") then prev = prev .. "
" end
		end
		writefile(USED_FILE, prev .. key .. "
")
	end)
end
...
local function savePendingKey(key, expireAt)
	pcall(function()
		if writefile then writefile(PENDING_FILE, key.."|"..tostring(expireAt)) end
	end)
...
local function isUnlocked()
	...
	-- Normal key: only valid if not expired
	if key and key ~= MASTER_KEY and expire then
		if os.time() <= expire then
			ENV.plus1_unlocked = true
			return true
		else
			-- Expired → clear session, must get a new key
	...
	-- Session flag alone is not enough if no valid key data
	ENV.plus1_unlocked = false
	return false
end

local function genKey()
	-- Never generate the master key
	...
	until out ~= MASTER_KEY and not isKeyUsed(out)
	local exp = os.time() + KEY_DURATION
	savePendingKey(out, exp)
	return out
end
...
mkL(KeyPage, "Enter your key below.\nGet key in-game • expires in 10 min • then earn a new one • Special key always works", 46, 11, Color3.fromRGB(150,150,150), false)
...
local function endSTD(success)
	...
	if success then
		playWin()
		earnedKey = genKey()
		-- pending already saved inside genKey; also show expire
...
SubmitBtn.MouseButton1Click:Connect(function()
	...
	-- ONLY accept keys earned from the game (pending from Spot the Difference)
	...
	if pending and entered == pending then
		...
		-- Accept game key for this session (10 min)
		saveKeyData(entered, os.time() + KEY_DURATION)
		-- Clear pending so they must earn a fresh key after this session expires
		pcall(function()
			if writefile then writefile(PENDING_FILE, "") end
		end)
		ENV.plus1_pending_key = nil
		ENV.plus1_pending_expire = nil
	...
	-- Still in an active session with this key?
...
-- Expiry watcher: when key expires, lock hub and force new key
task.spawn(function()
	while true do
		task.wait(5)
		...
		local key, expire = loadKeyData()
		if key == MASTER_KEY then
			-- master never expires for access
		elseif ENV.plus1_unlocked and key and expire and os.time() > expire then
			ENV.plus1_unlocked = false
			ENV.plus1_key = nil
			ENV.plus1_expire = nil
			...
			if Auth then
				Auth.Visible = true
				if LoadPage then LoadPage.Visible = false end
				if KeyPage then KeyPage.Visible = true end
				if RewardPage then RewardPage.Visible = false end
				if StatusLbl then
...
task.spawn(function()
	local ok, err = pcall(function()
		...
		for i = 1, 100 do
			if BarFill then BarFill.Size = UDim2.new(i / 100, 0, 1, 0) end
			if LoadPct then LoadPct.Text = i .. "%" end
			if i % 20 == 0 and LoadSub then
				LoadSub.Text = steps[math.clamp(math.floor(i / 20), 1, #steps)]
			end
			task.wait(0.02)
		end
		if LoadSub then LoadSub.Text = "Ready" end
		pcall(playClick)
		task.wait(0.25)
		if LoadPage then LoadPage.Visible = false end
		if KeyPage then KeyPage.Visible = true end
	...
	if not ok then
		warn("[+1 Scripts] load error:", err)
		-- Fallback: show main UI anyway so script is usable
		pcall(function()
			if Auth then Auth.Visible = false end
