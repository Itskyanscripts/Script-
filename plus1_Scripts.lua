...
-- Safe time (os.time can be nil in some executors)
...
local function shrinkPress(gui)
	if not gui then return end
	local orig = gui.Size
	pcall(function()
		gui.Size = UDim2.new(orig.X.Scale, math.max(orig.X.Offset - 6, 10), orig.Y.Scale, math.max(orig.Y.Offset - 4, 10))
		task.delay(0.08, function()
			if gui and gui.Parent then
				pcall(function()
					TweenService:Create(gui, TweenInfo.new(0.12, Enum.EasingStyle.Back), {Size = orig}):Play()
				end)
			end
		end)
	end)
...
local function saveRequest(user, msg)
	pcall(function()
		...
		writefile(REQUEST_FILE, existing .. user .. "|" .. msg:gsub("|", "/") .. "|" .. tostring(getTime()) .. "\n")
	end)
end
...
local function isUnlocked()
	...
	if key and key ~= MASTER_KEY and expire and getTime() <= expire then return true end
	return false
end
local function genKey()
	...
	savePendingKey(out, getTime() + KEY_DURATION)
	return out
end
...
mkL(KeyPage, "Get Key To Open", 18, 18, Color3.fromRGB(0, 220, 90), true)
...
SubmitBtn.MouseButton1Click:Connect(function()
	...
	if entered == MASTER_KEY then
		saveKeyData(MASTER_KEY, getTime() + KEY_DURATION)
	...
	if pending and entered == pending then
		if pendExp and getTime() > pendExp then
			StatusLbl.Text = "Key expired — play again for a new key"
			StatusLbl.TextColor3 = Color3.fromRGB(255, 100, 100)
			playExpire()
			return
		end
		saveKeyData(entered, getTime() + KEY_DURATION)
	...
	if saved and entered == saved and saved ~= MASTER_KEY and expire and getTime() <= expire then
		StatusLbl.Text = "Key accepted"
		StatusLbl.TextColor3 = Color3.fromRGB(0, 220, 90)
		task.wait(0.3)
		unlockHub()
...
-- Simple Spot the Difference (25 levels)
...
LeftPanel.Size = UDim2.new(0.47, -6, 1, -100)
...
RightPanel.Size = UDim2.new(0.47, -6, 1, -100)
RightPanel.Position = UDim2.new(0.53, 0, 0, 52)
...
RightTag.TextSize = 12
RightTag.ZIndex = 62
RightTag.Parent = RightPanel

local FoundLbl = Instance.new("TextLabel")
FoundLbl.Size = UDim2.new(1, -20, 0, 18)
FoundLbl.Position = UDim2.new(0, 10, 1, -24)
...
local function clearDiffs()
	...
	for _, c in ipairs(LeftPanel:GetChildren()) do
		if (c:IsA("TextLabel") or c:IsA("TextButton")) and c ~= LeftTag then
			c:Destroy()
		end
	end
	for _, c in ipairs(RightPanel:GetChildren()) do
		if (c:IsA("TextLabel") or c:IsA("TextButton")) and c ~= RightTag then
			c:Destroy()
		end
	end
end
		...
		if not overDiff then
			...
			for _, hit in ipairs(diffButtons) do
				if hit and hit.Parent then
					hit.Active = true
					for _, c in ipairs(hit:GetChildren()) do
...
task.spawn(function()
	while true do
		...
		local key, expire = loadKeyData()
		if key and key ~= MASTER_KEY and expire and getTime() > expire and Main.Visible then
			ENV.plus1_key = nil
