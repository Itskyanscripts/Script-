...
do
	...
	local infJump = false
	createToggle(HomePage, "Inf Jump", 116, function(v)
		infJump = v
	end)
	...
	-- Discord copy only (image icons, no number IDs shown)
	createSection(HomePage, "Discord", 146)
	...
	discRow.Size = UDim2.new(1, -4, 0, 36)
	discRow.Position = UDim2.new(0, 2, 0, 168)
	...
	discLbl.Size = UDim2.new(1, -44, 1, 0)
	discLbl.Position = UDim2.new(0, 8, 0, 0)
	...
	copyImg.Size = UDim2.new(0, 28, 0, 28)
	copyImg.Position = UDim2.new(1, -34, 0.5, -14)
	copyImg.BackgroundColor3 = Color3.fromRGB(70, 65, 140)
	copyImg.Image = ICON_COPY
	copyImg.AutoButtonColor = false
	...
	copyImg.MouseButton1Click:Connect(function()
		...
		if copyText("https://discord.gg/DvhPapJNm") then
			...
			copyImg.BackgroundColor3 = Color3.fromRGB(60, 160, 90)
			task.delay(1.5, function()
				if copyImg and copyImg.Parent then
					copyImg.Image = ICON_COPY
					copyImg.BackgroundColor3 = Color3.fromRGB(70, 65, 140)
				end
			end)
	...
	HomePage.CanvasSize = UDim2.new(0, 0, 0, 220)
end
...
do
	...
	pcall(function()
		if isfile and isfile("plus1_settings.txt") and readfile then
			local d = readfile("plus1_settings.txt")
			autoExec = d:find("autoexec=1") ~= nil
			autoRejoin = d:find("autorejoin=1") ~= nil
		end
	end)
	local function saveSettings()
		pcall(function()
			if writefile then
				writefile("plus1_settings.txt",
					"autoexec=" .. (autoExec and "1" or "0") .. "\n" ..
					"autorejoin=" .. (autoRejoin and "1" or "0"))
			end
		end)
	end
	createToggle(SettingsPage, "Auto Execute", 24, function(v)
		autoExec = v
		saveSettings()
		ENV.plus1_autoexec = v
	end)
	createToggle(SettingsPage, "Auto Rejoin When Kicked", 54, function(v)
		autoRejoin = v
		saveSettings()
		ENV.plus1_autorejoin = v
	end)
	-- apply initial toggle visuals by re-firing would need createToggle state; store flags
	if autoRejoin then
		pcall(function()
			game:GetService("CoreGui").RobloxPromptGui.promptOverlay.ChildAdded:Connect(function(c)
				if not ENV.plus1_autorejoin then return end
				task.wait(0.5)
				pcall(function()
		...
		player.OnTeleport:Connect(function() end)
		-- fallback: detect kick via PlayerRemoving is local - use GuiService
		pcall(function()
	...
	ENV.plus1_autoexec = autoExec
	ENV.plus1_autorejoin = autoRejoin
	SettingsPage.CanvasSize = UDim2.new(0, 0, 0, 100)
end
...
SliceGui.BorderSizePixel = 0
SliceGui.Visible = false
SliceGui.ZIndex = 85
SliceGui.Parent = ScreenGui
Instance.new("UICorner", SliceGui).CornerRadius = UDim.new(0, 12)
local sliceStroke = Instance.new("UIStroke", SliceGui)
sliceStroke.Color = Color3.fromRGB(255, 80, 80)
sliceStroke.Thickness = 3
...
heartsLbl.Size = UDim2.new(1, -16, 0, 28)
heartsLbl.Position = UDim2.new(0, 8, 0, 30)
heartsLbl.BackgroundTransparency = 1
heartsLbl.Text = "❤️ ❤️ ❤️"
heartsLbl.TextColor3 = Color3.fromRGB(255, 60, 60)
heartsLbl.Font = Enum.Font.Arcade -- pixelated arcade look
heartsLbl.TextSize = 22
heartsLbl.ZIndex = 86
heartsLbl.Parent = SliceGui

local sliceArea = Instance.new("Frame")
sliceArea.Size = UDim2.new(1, -16, 0, 180)
sliceArea.Position = UDim2.new(0, 8, 0, 60)
...
sliceStatus.Size = UDim2.new(1, -16, 0, 20)
sliceStatus.Position = UDim2.new(0, 8, 0, 248)
sliceStatus.BackgroundTransparency = 1
sliceStatus.Text = "Click animals to slice!"
...
makeDraggable(sliceTitle, SliceGui)

local spotLevel = 1
local sliceHearts = 3
local sliceRunning = false
local sliceNeed = 12 -- animals to slice to win
local sliceCount = 0
...
local function finishGetKeyWin()
	-- random letters+numbers, max 13 chars — not shown on screen
...
local function loseHeart()
	...
	if sliceHearts <= 0 then
		sliceRunning = false
		sliceStatus.Text = "No hearts left!"
		task.wait(0.5)
		pcall(function() player:Kick("Ran Out Of Time!") end) -- hearts depleted kick
	end
end

local function spawnAnimal()
	if not sliceRunning or not SliceGui.Visible then return end
	local emoji = ANIMAL_EMOJIS[math.random(1, #ANIMAL_EMOJIS)]
	local btn = Instance.new("TextButton")
	btn.Size = UDim2.new(0, 36, 0, 36)
	btn.Position = UDim2.new(0, math.random(10, 250), 1, 0)
	btn.BackgroundTransparency = 1
	btn.Text = emoji
	btn.TextSize = 28
	btn.Font = Enum.Font.GothamBold
	...
	btn.MouseButton1Click:Connect(function()
		...
		playClick()
		btn.Text = "✓"
		btn.TextColor3 = Color3.fromRGB(255, 40, 40) -- red check
		...
		if sliceCount >= sliceNeed then
			sliceRunning = false
			sliceStatus.Text = "You win!"
			task.delay(0.6, finishGetKeyWin)
		end
	end)
	-- throw up then fall
	task.spawn(function()
		local peak = math.random(20, 90)
		...
		for y = peak, 200, 5 do
			if not btn.Parent then return end
			btn.Position = UDim2.new(0, x + math.sin(y/20)*8, 0, y)
			task.wait(0.02)
		end
		if btn.Parent and not sliced then
			btn:Destroy()
			if sliceRunning then loseHeart() end
		end
	end)
end
...
local function startSliceGame()
	...
	sliceRunning = true
	updateHearts()
	sliceStatus.Text = "Click animals to slice! " .. sliceCount .. " / " .. sliceNeed
...
local function startSpotLevel(lv)
	...
	local positions = {}
	for i = 1, 5 do
		local x = 8 + ((seed * i * 7) % 90)
		local y = 8 + ((seed * i * 11) % 110)
		local em = SPOT_EMOJIS[((i + lv - 1) % #SPOT_EMOJIS) + 1]
		local function addEmoji(parent, xx, yy, e)
			local b = Instance.new("TextLabel")
			b.Size = UDim2.new(0, 28, 0, 28)
			b.Position = UDim2.new(0, xx, 0, yy)
		...
		addEmoji(leftPane, x, y, em)
		addEmoji(rightPane, x, y, em)
	end
	-- difference only on right
	local dx = 12 + ((seed * 13) % 90)
	local dy = 12 + ((seed * 19) % 100)
	local dem = SPOT_EMOJIS[((lv + 3) % #SPOT_EMOJIS) + 1]
	local diffBtn = Instance.new("TextButton")
	diffBtn.Size = UDim2.new(0, 30, 0, 30)
	diffBtn.Position = UDim2.new(0, dx, 0, dy)
	diffBtn.BackgroundTransparency = 1
	diffBtn.Text = dem
	diffBtn.TextSize = 24
	diffBtn.ZIndex = 83
	...
	diffBtn.MouseButton1Click:Connect(function()
		...
		if spotLevel >= TOTAL_LEVELS then
			...
			task.delay(0.5, startSliceGame)
		else
			spotStatus.Text = "Correct!"
			task.delay(0.3, function() startSpotLevel(spotLevel + 1) end)
		end
	end)
end
...
sliceClose.MouseButton1Click:Connect(function()
	playClick()
	sliceRunning = false
	SliceGui.Visible = false
	InsertKey.Visible = true
...
confirmBtn.MouseButton1Click:Connect(function()
	...
	-- minigame keys already stored in VALID_KEYS when won
	if not info then
